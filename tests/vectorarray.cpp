...
pybind11::array_t<F> create_iota_python_numpy_array(ssize_t size, ssize_t dim, F start = F(1))
{
        ...
        for (ssize_t size : {0, 1, 3, 4})
        {
            for (ssize_t dim : {0, 1, 3, 4})
            {
                    ...
                    [size, dim]
                {
                    ...
                    {
                        const auto py_array = create_iota_python_numpy_array<F>(size, dim);
                        expect(nothrow(
                            [&]()
                            {
                    ...
                    const auto v = NumpyVectorArray<F>(create_iota_python_numpy_array<F>(size, dim));
                    test("Copying") = [=]()
                    {
                        ...
                        if (size > 0 && dim > 0)
                        {
                            dynamic_cast<NumpyVectorArray<F>*>(v_copy.get())->array().mutable_at(0, 0) =
                                F(42);
                                                 ...
                                                 std::vector<ssize_t> {-1, 0, 1}})
                        {
                            ...
                            if (size > 0 && dim > 0)
                            {
                                dynamic_cast<NumpyVectorArray<F>*>(v_copy_with_indices.get())
                                    ->array()
                                    .mutable_at(0, 0) = F(42);
                                expect(exactly_equal(v.get(0, 0), F(1))) << "copy is deep";
                            }
                    ...
                    test("append") = [=]()
                    {
                        ...
                        auto v2 = NumpyVectorArray<F>(create_iota_python_numpy_array<F>(v2_size, dim));
                    ...
                    test("axpy") = [=]()
                    {
                        ...
                        auto v_copy2 = v.copy();
                        const auto x =
                            NumpyVectorArray<F>(create_iota_python_numpy_array<F>(size, dim, F(-10)));
                        std::vector<F> alpha(size, F(42));
                        expect(fatal(nothrow(
                            [&]()
                            {
                              ...
                              std::set<ssize_t> {-1, 0, std::min(ssize_t(1), size - 1)}})
                        {
                                  ...
                                  std::set<ssize_t> {-1, 0, std::min(ssize_t(1), size - 1)}})
                            {
                                ...
                                auto x_copy = same_size
                                                  ? x.copy()
                                                  : NumpyVectorArray<F>(create_iota_python_numpy_array<F>(
