...
def merge_replacement_files(tmpdir, mergefile):
    ...
    if merged:
        # deduplicate entries
        merged_without_duplicates = []
        for item in merged:
            if item not in merged_without_duplicates:
                merged_without_duplicates.append(item)
        # MainSourceFile: The key is required by the definition inside
        # include/clang/Tooling/ReplacementsYaml.h, but the value
        # is actually never used inside clang-apply-replacements,
        # so we set it to '' here.
        output = {"MainSourceFile": "", mergekey: merged_without_duplicates}
...
def main():
    ...
    parser.add_argument("-enable-check-profile", action="store_true", help="Enable per-check timing profiles")
    ...
    parser.add_argument(
        ...
        default=[],
        action="append",
        help="Override diff behaviour and match files in given dir against -regex instead of using the diff.",
    )
    parser.add_argument(
        "-diff-file",
        dest="diff_file",
    ...
    if args.regex is not None:
        regex = re.compile("^%s$" % args.regex)
    else:
        regex = re.compile("^%s$" % args.iregex, re.IGNORECASE)

    # Extract changed lines for each file.
    filename = None
    lines_by_file = {}
    if args.source_dir:
        print("Source dir mode. Ignoring diff from stdin")
        for source_dir in args.source_dir:
            for file in Path(source_dir).rglob("*"):
                if regex.match(file.name):
                    lines_by_file[str(file)] = None

    else:
        if args.diff_file:
            diff = Path(args.diff_file).read_text().splitlines()
        else:
            diff = sys.stdin
        for line in diff:
            match = re.search('^\\+\\+\\+\\ "?(.*?/){%s}([^ \t\n"]*)' % args.p, line)
            ...
            if match:
                ...
                lines_by_file.setdefault(os.path.normpath(filename), []).append([start_line, end_line])
    ...
    if args.enable_check_profile:
        common_clang_tidy_args.append("--enable-check-profile")
    ...
    for name in lines_by_file:
        ...
        if lines_by_file[name] is not None:
            line_filter_json = json.dumps(
                # The original code here was
                # [{"name": name, "lines": lines_by_file[name]}], separators=(",", ":")
                # However, we noticed that some issues were not reported as expected.
                # In particular, unused or missing headers were often not recognized.
                # This makes sense since the error in these cases was not in the changed lines.
                # However, we still want to get these warnings, so we remove the line numbers here
                # and basically turn the line filter into a file filter.
                [{"name": name}],
                separators=(",", ":"),
            )
